            ...
            if filename.endswith(".csv"):
                ...
                with open(filepath, newline=newline) as file:
                    ...
                    total_run_spd_reward = np.append(
                        total_run_spd_reward, run_speed_sum_reward_sum
                    )
                    total_energy_cons_reward = np.append(
                        total_energy_cons_reward, energy_cons_reward_sum
                    )
                link_lenghts[directoryname] = {"link_lenghts": link_lenghts_ind}
                value_sums_mean[directoryname] = {
                    "running_speed_returns_sum_mean": np.mean(total_run_spd_reward),
                    "energy_consumption_returns_sum_mean": np.mean(
                        total_energy_cons_reward
                    ),
                }
                value_sums[directoryname] = {
                    "running_speed_returns_sum": total_run_spd_reward,
                    "energy_consumption_returns_sum": total_energy_cons_reward,
                }
...
value_std = {
    index: [
        np.std(value_sums[index]["running_speed_returns_sum"], axis=0),
...
running_speed_sums = [
    item["running_speed_returns_sum_mean"] for item in value_sums_mean_sorted.values()
]  # ORIG
energy_cons_sums = [
    item["energy_consumption_returns_sum_mean"]
    for item in value_sums_mean_sorted.values()
]
...
ax.errorbar(
    index - off_set,
    [value_sums_mean[index]["running_speed_returns_sum_mean"] for index in key_order],
    yerr=[value_std[index][0] for index in key_order],
    fmt="none",
    color="black",
    capsize=7,
)
ax.errorbar(
    index + off_set,
    [
        value_sums_mean[index]["energy_consumption_returns_sum_mean"]
        for index in key_order
    ],
...
for index, weight in enumerate(key_order):
    ax2.annotate(
        key_order[index],
        (running_speed_sums[index], energy_cons_sums[index]),
