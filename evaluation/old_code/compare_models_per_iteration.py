...
def sort_dictionaries(path):
    ...
    for directoryname in os.listdir(path):
        ...
        if not directoryname.split("_")[-1] == "baseline":
            ...
            directory_key_baseline = (
                directoryname.split("__")[-1]
                .replace(",", ", ")
                .split("[")[-1:][0]
                .split("_")[0]
                .replace("]", "")
                ...
                for filename in os.listdir(directorypath):
                    if filename.endswith(".csv"):
                        total_run_spd_reward = np.array(
                            []
                        )  # reset when in new csv file
                        total_energy_cons_reward = np.array([])
                        link_lenghts_ind = np.array([])
                        filename_key = filename.split("_")[-1].split(".")[
                            0
                        ]  # set ind key for each csv file
                        ...
                        with open(filepath, newline=newline) as file:
                            ...
                            for row in reader:
                                rows.append(row)
                            link_lenghts_ind = np.append(
                                link_lenghts_ind, np.array(rows[0], dtype=float)
                            )
                            total_run_spd_reward = np.append(
                                total_run_spd_reward, np.array(rows[1], dtype=float)
                            )
                            total_energy_cons_reward = np.append(
                                total_energy_cons_reward, np.array(rows[2], dtype=float)
                            )
                            # values to dict
                    value_mean[directory_key][filename_key] = {
                        "running_speed_returns_mean": np.mean(total_run_spd_reward),
                        "energy_consumption_returns_mean": np.mean(
                            total_energy_cons_reward
                        ),
                    }
                    link_lengths[directory_key][filename_key] = link_lenghts_ind
                    value_std[directory_key][filename_key] = {
                        "running_speed_returns_std": np.std(total_run_spd_reward),
                        "energy_consumption_returns_std": np.std(
                            total_energy_cons_reward
                        ),
                    }
            else:
                for filename in os.listdir(directorypath):
                    if filename.endswith(".csv"):
                        ...
                        with open(filepath, newline=newline) as file:
                            ...
                            # values to dict
                    value_mean_baseline[directory_key_baseline][filename_key] = {
                        "running_speed_returns_mean": np.mean(total_run_spd_reward),
                        "energy_consumption_returns_mean": np.mean(
                            total_energy_cons_reward
                        ),
                    ...
                    value_std_baseline[directory_key_baseline][filename_key] = {
                        "running_speed_returns_std": np.std(total_run_spd_reward),
                        "energy_consumption_returns_std": np.std(
                            total_energy_cons_reward
                        ),
...
def line_plot():
    ...
    unique_weight_groups = sorted(
        set(sorted_mean.keys()),
        key=lambda item: convert_key_to_tuple(item.split("_")[0]),
    )
    _, axes = plt.subplots(
        len(unique_weight_groups), 2, figsize=(10, 10)
    )  # * len(unique_weight_groups)))
    adjusted_color_dict = {
        weight_group: plt.get_cmap("plasma")(i / len(unique_weight_groups))
        for i, weight_group in enumerate(unique_weight_groups)
    }
    alp_value = 0.8
    ...
    for i, weight in enumerate(unique_weight_groups):
        ...
        axes[i, 0].plot(
            range(reward_mean.shape[1]),
            reward_mean[weight_index, :, 0],
            color=color,
            alpha=alp_value,
            label=weight_group,
            linewidth=2,
        )  # models per iteration to compare
        axes[i, 0].plot(
            range(0, 60),
            reward_mean_baseline[:, 0],
            color="darkorange",
            alpha=alp_value,
            label="baseline",
            linewidth=2,
...
def link_length_plot(
    save_file: bool = False, save_dir="link_length_comparison_results"
):
    """link length plot
    Set to 'True' to save plots into html files or 'False' to not save
    """
    weight_categories = list(sorted_link_lengths.keys())
    distinct_error_colors = list(
        plt.get_cmap("plasma")(i / len(weight_categories))
        for i, _ in enumerate(weight_categories)
    )  # viridis_r
    ...
    for j in range(link_lengths_array.shape[2]):
        ...
        for i, weight_category in enumerate(weight_categories):
            ...
            figm.update_layout(
                ...
                xaxis=dict(
                    title="weights", tickvals=index_weigths, ticktext=weight_categories
                ),
        ...
        if save_file:
            ...
            html_file_path_mean = os.path.join(
                save_dir, f"Link_length_{j + 1}_mean_values.html"
            )
            figm.write_html(html_file_path_mean)
            print(f"Figure saved as {html_file_path_mean}")
            html_file_path_reg = os.path.join(
                save_dir, f"Link_length_{j + 1}_values.html"
            )
            figv.write_html(html_file_path_reg)
            print(f"Figure saved as {html_file_path_reg}")
            # save the means as pdf
            pdf_file_path_mean = os.path.join(
                save_dir,
                f'Link_length_{j + 1}_mean_values_{save_file_name.split("_")[-1]}_.pdf',
            )
            figm.write_image(pdf_file_path_mean, format="pdf")
            print(f"Figure saved as {pdf_file_path_mean}")
            # save the values as pdf
            pdf_file_path_values = os.path.join(
                save_dir,
                f'Link_length_{j + 1}_values_{save_file_name.split("_")[-1]}_.pdf',
            )
...
if __name__ == "__main__":
    ...
    (
        ...
        path
    )  # If you're analysing models trained with pre-trained model, pass 'True' else 'False'

    # Sort dictionaries based on keys
    sorted_mean = dict(
        sorted(
            value_mean.items(),
            key=lambda item: convert_key_to_tuple(item[0].split("_")[0]),
        )
    )
    sorted_std = dict(
        sorted(
    ...
    sorted_mean_baseline = dict(
        sorted(
            value_mean_baseline.items(),
            key=lambda item: convert_key_to_tuple(item[0].split("_")[0]),
        )
    )
    sorted_std_baseline = dict(
        sorted(
            value_std_baseline.items(),
            key=lambda item: convert_key_to_tuple(item[0].split("_")[0]),
        )
    )
    sorted_link_lengths_baseline = dict(
        sorted(
            link_lengths_baseline.items(),
            key=lambda item: convert_key_to_tuple(item[0].split("_")[0]),
        )
    )  # Sort link lengths
    ...
    for key, inner_dict in sorted_mean_baseline.items():
        ...
        sorted_mean_baseline[key] = dict(
            sorted(inner_dict.items(), key=lambda item: convert_key_to_tuple(item[0]))
        )
    # sums
    for key, inner_dict in sorted_std_baseline.items():
        sorted_std_baseline[key] = dict(
            sorted(inner_dict.items(), key=lambda item: convert_key_to_tuple(item[0]))
        )
    # link lengths
    for key, inner_dict in sorted_link_lengths_baseline.items():
        sorted_link_lengths_baseline[key] = dict(
            sorted(inner_dict.items(), key=lambda item: convert_key_to_tuple(item[0]))
        )
    ...
    for key, inner_dict in sorted_link_lengths.items():
        sorted_link_lengths[key] = dict(
            sorted(inner_dict.items(), key=lambda item: convert_key_to_tuple(item[0]))
        )

    # Calculate values to arrays
    # baseline
    reward_mean_baseline = np.array(
        [
            (
                sorted_mean_baseline[key1][key2]["running_speed_returns_mean"],
                sorted_mean_baseline[key1][key2]["energy_consumption_returns_mean"],
            )
            for key1 in sorted_mean_baseline.keys()
            for key2 in sorted_mean_baseline[key1].keys()
        ]
    )

    reward_std_baseline = np.array(
        [
            (
                sorted_std_baseline[key1][key2]["running_speed_returns_std"],
                sorted_std_baseline[key1][key2]["energy_consumption_returns_std"],
            )
            for key1 in sorted_std_baseline.keys()
            for key2 in sorted_std_baseline[key1].keys()
        ]
    )
    ...
    reward_mean = np.array(
        [
            [
                (
                    sorted_mean[key1][key2]["running_speed_returns_mean"],
                    sorted_mean[key1][key2]["energy_consumption_returns_mean"],
                )
                for key2 in sorted_mean[key1].keys()
            ]
            for key1 in sorted_mean.keys()
        ]
    )
    ...
    reward_std = np.array(
        [
            [
                (
                    sorted_std[key1][key2]["running_speed_returns_std"],
                    sorted_std[key1][key2]["energy_consumption_returns_std"],
                )
                for key2 in sorted_std[key1].keys()
            ]
            for key1 in sorted_mean.keys()
        ]
    )
    ...
    # baseline
    link_lengths_array = np.array(
        [
            [weights[key] for key in weights]
            for weights in sorted_link_lengths_baseline.values()
        ]
    )
    ...
    # model values
    link_lengths_array = np.array(
        [[weights[key] for key in weights] for weights in sorted_link_lengths.values()]
    )
    link_lengths_mean_array = np.array(
        [
            np.mean(list(weights.values()), axis=0)
            for weights in sorted_link_lengths.values()
        ]
    )
    link_lengths_std_array = np.array(
        [
            np.std(list(weights.values()), axis=0)
            for weights in sorted_link_lengths.values()
        ]
    )
    ...
    sem_link_length = link_lengths_std_array / np.sqrt(
        56
    )  # 5 for vect or 7 for steered models per weight # sample size (amount of trained models per weight and seed) -> regular model or (trained models per loaded weight in weight) -> loaded model
