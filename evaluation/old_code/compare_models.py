        ...
        if os.path.isdir(directorypath):
            for directoryname2 in os.listdir(directorypath):
                ...
                if not loaded:
                    ...
                    directory_keyname = (
                        directory_keyname[0].replace("]", "_").replace(", ", "_")
                ...
                if os.path.isdir(directorypath2):
                        ...
                        if filename.endswith(".csv"):
                            ...
                            with open(filepath, newline=newline) as file:
                                ...
                                total_energy_cons_reward = np.append(
                                    ...
                                    np.array(rows[2], dtype=float),
                                )
                                # values to dict
                    value_mean[directoryname][directory_keyname] = {
                    ...
                    value_std[directoryname][directory_keyname] = {
...
def bar_plot(steered: bool = False):
    ...
    bar2 = ax.bar(
        index + off_set, reward_mean[:, 1], bar_width, label="Energy Consumption"
    )
    ...
    ax.set_title(
        "Mean episodic returns for each weight of Running Speed and Energy Consumption for Each Weight"
    )
    ax.set_xticks(index)
    ...
    if steered:
        ax.set_xticklabels(
            ...
            rotation=45,
            ha="right",
        )  # ADD both keys to names
    else:
        ax.set_xticklabels(
            [key2 for key1 in sorted_mean.keys() for key2 in sorted_mean[key1].keys()],
            rotation=45,
            ha="right",
        )  # ADD both keys to names
    ax.legend()
...
def scatter_plot():
    ...
    for index, (key1, key2) in enumerate(
        [
            (key1, key2)
            for key1 in sorted_mean.keys()
            for key2 in sorted_mean[key1].keys()
        ...
        weight_group = unique_weight_groups[
            np.where(mask)[0][0]
        ]  # Find the index where the mask is True
    ...
    for index, txt in enumerate(
        [key2 for key1 in sorted_mean.keys() for key2 in sorted_mean[key1].keys()]
    ):
        ax2.annotate(
            txt,
            (reward_mean[index, 0], reward_mean[index, 1]),
...
if __name__ == "__main__":
    ...
    value_mean, value_std, link_lengths = sort_dictionaries(
        path, True
    )  # If you're analysing models trained with pre-trained model, pass 'True' else 'False'
    # Sort dictionaries based on keys
    sorted_mean = dict(
        sorted(value_mean.items(), key=lambda item: convert_key_to_tuple(item[0]))
    )
    sorted_std = dict(
        sorted(value_std.items(), key=lambda item: convert_key_to_tuple(item[0]))
    )
    sorted_link_lengths = dict(
        sorted(link_lengths.items(), key=lambda item: convert_key_to_tuple(item[0]))
    )  # Sort link lenghts
    ...
    ci_running_speed = confidency_interval * (
        np.array([reward_std[i][0] for i in range(len(reward_std))])
        / np.sqrt(sample_count)
    )  # sample size is 5 since we have 5 different test runs  #np.sqrt(len(reward_std)))
    ci_energy_consumption = confidency_interval * (
        np.array([reward_std[i][1] for i in range(len(reward_std))])
        / np.sqrt(sample_count)
    )

    sem_link_length = link_lengths_std_array / np.sqrt(
        14
    )  # if done together then its 14 #5 for vect or 7 for steered models per weight # sample size (amount of trained models per weight and seed) -> regular model or (trained models per loaded weight in weight) -> loaded model
