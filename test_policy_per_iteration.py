...
path_to_folder = '/home/oskar/Thesis/inter/models_vect_batch/steered/0.6_0.4/Wed_Feb_21_18:37:36_2024__0.8,0.2_3_[0.6,0.4]_3' 
#path_to_folder = '/home/oskar/Thesis/inter/models_vect_batch/set_seed/0.8_0.2/Sat_Jan_20_00_44_42_2024__a67b47d7[0.8,0.2]_3' # provided model path for testing multiple iterations of the model
...
def testing(model_path, iteration : int ,count : int = 0, save_returns_mode : bool = True):
    ...
    last_model_checkpoint_num = iteration#find_checkpoint(model_path)#-1 # checkpoint
    ...
    file_str = './' + folder # + '/' + '__' + model_name + '_chkpnt_' + str(last_model_checkpoint_num) # + '_run_' + str(count+1) save csv fiels to batch folder per iteration
    ...
    with open(
            os.path.join(file_path,
                ...
                '{}{}_{}.csv'.format(file_name, run_name, str(iteration))
                ), 'w') as fd:
        #simulate the model
...
if __name__ == "__main__": 
    #episodic returns per iteration per model checkpoint
    cnt = find_checkpoint(path_to_folder)
    for iter in range(cnt):
        testing(path_to_folder, iter+1)